...
function solve_model(a0_initial::Float64, a1_initial::Float64, model::model_params, grid::grids, shock::shocks, tm::transition_matrices, sim::sim_param)
    difftrans = 10
    iteration = 0;
    a0 = a0_initial
    a1 = a1_initial
    ...
    while difftrans > 0.01
        ...
        println("KS iteration number ", iteration)
        ...
        Vnoadjustnew, Vadjustnew, policy, policyadjust = solve_policy(grid, model, tm, χ̂, a0, a1);
        
        println("Now simulating the model")
        
        # Simulate
        Csim, χsim, frequp_t, freqdown_t, sizeup_t, sizedown_t = simulate_panel(grid, sim, model, tm, χ̂, policy, shock, fshocks);
    ...
    return policy, policyadjust, a0, a1, χ̂
end
